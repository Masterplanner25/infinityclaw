...
class _AsyncAINDYClient:
    ...
    async def emit_event(self, event_type: str, payload: dict[str, Any] | None = None) -> dict[str, Any]:
        # aindy-sdk >= 1.0.1 sends the key the runtime reads (`event_type`); 1.0.0 sent `type`
        # and 422'd on every call. The `[aindy]` extra floors the SDK at 1.0.1 for this reason,
        # and tests/test_aindy_wire.py pins the wire shape against the runtime's own schema.
        return await asyncio.to_thread(self._sync.events.emit, event_type, payload or {})
